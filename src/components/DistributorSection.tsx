  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveFeature((prev) => 
        (prev + 1) % ADDITIONAL_DISTRIBUTOR_FEATURES.length
      );
    }, 5000);
            ...
            <div className="space-y-4 order-2 md:order-1">
              {ADDITIONAL_DISTRIBUTOR_FEATURES.map((feature, index) => (
                ...
                >
                  ...
                  {activeFeature === index && (
                    <p className="text-[#5F6368] text-[16px] font-[400] md:w-[50%] animate-in fade-in slide-in-from-left-2 duration-300">{feature.description}</p>
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="bg-[#F6F3F3] flex items-center justify-center rounded-[16px] min-h-[300px] p-8 order-1 md:order-2">
              <div className="text-center">
                ...
                <p className="text-gray-600">
                  {ADDITIONAL_DISTRIBUTOR_FEATURES[activeFeature].description}
                </p>
              </div>
            </div>
          </div>
